...
```{r}
table(con, 'hotel') %>% 
  inner_join(table(con, 'review'), by = c('hotelId' = 'hotelId')) %>% 
  group_by(hotelName) %>% 
  summarise()
```
...
```{r}
table(con, 'hotel') %>% 
  inner_join(table(con, 'review')) %>% 
  inner_join(table(con, 'author')) %>% 
  select(authorNickname, hotelName, hotelId) %>% 
  arrange(desc(authorName))
```
