...
```{r}
ggplot(data = selfiesCasualties) + geom_bar(aes(x = class, fill = gender))

ggplot(data = selfiesCasualties) + geom_bar(aes(x = class, y =..prop.., fill = gender, position = 'dodge'))
```
...
```
```{r}
ggplot(data = dt) + geom_bar(aes(x = College, y = ..prop.., fill = Sex, group = 1))
```
...
```
```{r}
ggplot(data = dt) + geom_bar(aes(x = College, y = ..prop.., fill = Sex), position = fill)
```
...
```
```{r}
ggplot(data = dt) + geom_bar(aes(x = College, fill = Sex)) + facet_wrap(~Ethnicity, scales = 'free_y')
```


```{r}
#repeat the previous but facetting by College and mapping ethnicity to x
# add:  "A subtitle", caption = 'data from www.lsu.edu')
```
```{r}
ggplot(data = dt) + geom_bar(aes(x = Ethnicity, fill = Sex)) + facet_wrap(~College, scales = 'free_y') + labs(title = "New plot title", subtitle = "A subtitle", caption = 'data from www.lsu.edu')
```
...
ggplot(data = dt) + geom_bar(aes(College, fill = Sex)) + scale_fill_brewer(palette = 'Reds', name = 'Gender')
```
...
```
```{r}
ggplot(data = dt) + geom_bar(aes(College, fill = Sex)) + scale_fill_brewer(palette = 'Greens', name = 'Gender') + theme(axis.text.x = element_text(angle = 45, hjust = 1),
